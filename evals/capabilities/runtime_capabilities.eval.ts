...
describe("Runtime Capability Coverage Eval", () => {
  it("records deterministic capability evidence for formerly stubbed features", async () => {
    await recordCapabilityCase("device_actions_real_driver_contract", "Device actions route through non-mock driver contracts", async () => {
      const allowlist = buildDeviceActionAllowlist({
        browser: { enabled: false },
        desktopInput: { enabled: false },
        android: { enabled: true, allowlist: { resources: ["emulator-5554"], risks: ["readonly", "safe_interaction"] } },
      } as any);
    ...
    await recordCapabilityCase("obs_websocket_control_contract", "OBS control exposes real status/start/stop/scene tools and disabled fail-closed behavior", async () => {
      ...
      return {
        output: {
          tools: ["obs.status", "obs.start_stream", "obs.stop_stream", "obs.set_scene"].map(name => Boolean(registry.get(name))),
          disabledStart,
        },
        checks: [
          ...
          { ok: disabledStart.ok === false && disabledStart.summary.includes("disabled"), name: "obs_disabled_fail_closed" },
        ],
      };
    });

    await recordCapabilityCase("live_emotion_expression_mapping", "Live emotions map to concrete Live2D expression and motion commands", async () => {
      const stageOutput = {
      ...
      return {
        ...
        checks: [
          { ok: intent.output.expression === "exp_02", name: "laughing_expression_mapped", note: `expression=${intent.output.expression}` },
          { ok: intent.output.motion === "TapBody", name: "laughing_motion_mapped", note: `motion=${intent.output.motion}` },
        ],
      };
    });

    await recordCapabilityCase("inner_memory_writer_persistence", "InnerMemoryWriter persists self state, research logs, and identity proposals", async () => {
      const memory = {
      ...
      await writer.proposeIdentityChange({ id: "p1", change: "identity proposal", rationale: "test", confidence: 0.7 });
      return {
        output: {
