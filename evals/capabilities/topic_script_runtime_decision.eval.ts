...
describe.skipIf(!hasEvalLlmKeys())("Topic Script Runtime Decision LLM Eval", () => {
  it("routes online signals without live tool calls", async () => {
    ...
    for (const evalCase of cases) {
      ...
      const allowed = Array.isArray(expected.allowedActions)
        ? expected.allowedActions.map(String)
      ...
      const score = summarizeChecks([
        ...
        {
          ok: allowed.includes(result.action),
          name: "expected_action",
          note: `action=${result.action}; allowed=${allowed.join(",")}`,
        },
