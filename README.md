...
│       ├── client/     # 渲染器前端源码
│       ├── models/     # Live2D 模型文件，默认使用 白-免费版
│       └── vendor/     # Live2D/Cubism 本地运行时资源
...
前端运行时通过 `/models/白-免费版/白-免费版.model3.json` 访问该模型。若要更换默认模型，请把新的模型目录放到 `assets/renderer/models/` 下，并更新 `assets/renderer/client/src/live2d.ts` 中的 `DEFAULT_MODEL_URL`。
