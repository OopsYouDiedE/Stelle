...
async function analyze() {
  ...
  console.log("Users talking to/about Stelle most:", Object.entries(userCounts).sort((a, b) => b[1] - a[1]).slice(0, 5));

  // Sample feedback
  console.log("\nSample Feedback/Impressions (Middle to End):");
  mentions.slice(Math.floor(mentions.length / 2), Math.floor(mentions.length / 2) + 30).forEach(m => console.log(`[${m.authorName}]: ${m.content}`));
}
