...
import { PolicyOverlayStore, type PersonaState } from "../policy_overlay_store.js";
import { type LiveEmotion, type LiveToolResultView, type LiveOutputProposal, ProposalPriority } from "./types.js";
...
export class LiveDanmakuCursor extends BaseStatefulCursor {
  ...
  protected async onInitialize(): Promise<void> {
    ...
    this.unsubscribes.push(
      this.context.eventBus.subscribe("stage.output.completed", (event) => {
    ...
    this.unsubscribes.push(
      this.context.eventBus.subscribe("live.output.proposal", (event) => {
        void this.receiveProposal(event).catch((e) => console.error("[LiveCursor] Proposal error:", e));
      }),
    );

    this.unsubscribes.push(
      this.context.eventBus.subscribe("live.topic.transition", (event) => {
  ...
  private async receiveProposal(event: StelleEvent) {
    ...
    const priority =
      source === "engagement_thanks" &&
      (intent.metadata?.eventKind === "super_chat" || intent.metadata?.eventKind === "guard")
        ? ProposalPriority.URGENT
        : source === "engagement_thanks" && intent.metadata?.eventKind === "gift"
          ? ProposalPriority.HIGH
          : source === "live_program"
            ? ProposalPriority.STRATEGIC
    ...
    console.log(
      `[LiveCursor] Buffered proposal ${proposal.id} (Priority: ${priority}) total=${this.proposalBuffer.length}`,
    );
  ...
  async receiveDirectSay(event: StelleEvent): Promise<{ accepted: boolean; reason: string; eventId: string }> {
    ...
    if (text) {
      const decisions = await this.responder.enqueue(forceTopic ? "topic" : "response", text, "neutral", {
  ...
  async receiveTopicRequest(event: StelleEvent): Promise<{ accepted: boolean; reason: string; eventId: string }> {
    if (event.type !== "live.topic_request" && event.type !== "live.request")
      return { accepted: false, reason: "Invalid type", eventId: event.id };
  ...
  async receiveLiveEvent(payload: Record<string, unknown>) {
    ...
    const cmd = String(
      payload.cmd ?? (payload.raw && typeof payload.raw === "object" ? (payload.raw as any).cmd : "") ?? "live_event",
    );
  ...
  private containsUrgentEvent(batch: NormalizedLiveEvent[] | undefined): boolean {
    return Boolean(
      batch?.some(
        (event) =>
          event.kind === "super_chat" || event.kind === "guard" || event.kind === "gift" || event.priority === "high",
      ),
    );
  }
  ...
  private async handleBatch(batch: NormalizedLiveEvent[]): Promise<void> {
    ...
    try {
      console.log(
        `[LiveCursor] processing ${batchId} size=${events.length} proposals=${this.proposalBuffer.length} oldestAgeMs=${oldestAgeMs}`,
      );
      const activePolicies = this.policyStore.activePolicies("live_danmaku");
      const personaState = this.policyStore.getPersonaState("live_danmaku");

      // 1. 决策 (Router)
      this.summary = "Designing live strategy...";
      let decision = await this.router.decide(
        events,
        this.responder.getRecentSpeech(),
        this.currentEmotion,
        activePolicies,
        this.proposalBuffer,
        personaState,
      ...
      if (decision.action !== "drop_noise" && decision.script.trim()) {
        ...
        console.log(
          `[LiveCursor] stage output ${decisions.map((item) => item.status).join(",") || "none"}: ${truncateText(decision.script, 80)}`,
        );
  ...
  private async maybeExecuteFallbackProposal(decision: any) {
    ...
    if (top.priority >= ProposalPriority.URGENT) {
