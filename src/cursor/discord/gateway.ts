...
export class DiscordGateway {
  ...
  public async filterAndBuffer(
    ...
    onReady: (session: DiscordChannelSession, batch: DiscordMessageSummary[], isDirectMention: boolean) => Promise<void>
  ...
  private async getBotUserId(): Promise<string | null> {
    ...
    const result = await this.context.tools.execute("discord.status", {}, { caller: "system", cwd: process.cwd(), allowedAuthority: ["readonly"], allowedTools: ["discord.status"] });
