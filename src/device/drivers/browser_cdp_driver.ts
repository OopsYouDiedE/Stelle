...
export class BrowserCdpDriver implements DeviceActionDriver {
  ...
  private async executeOnTarget(client: CdpClient, intent: DeviceActionIntent): Promise<Record<string, unknown>> {
    ...
    switch (intent.actionKind) {
      ...
      case "type":
        ...
        return { resourceKind: this.resourceKind, actionKind: intent.actionKind, textLength: String(payload.text ?? "").length };
      case "hotkey":
  ...
  private async resolveTarget(intent: DeviceActionIntent): Promise<CdpTarget> {
    const payload = asRecord(intent.payload);
    const directWs = asString(payload.webSocketDebuggerUrl) ?? (intent.resourceId.startsWith("ws") ? intent.resourceId : undefined);
    ...
    if (!target?.webSocketDebuggerUrl) {
      throw new Error(`No CDP target found. Start Chrome/Edge with --remote-debugging-port=9222 or pass payload.webSocketDebuggerUrl.`);
    }
    return target;
  }
...
class CdpClient {
  ...
  private constructor(private readonly ws: any, private readonly timeoutMs: number) {
...
function normalizeKey(key: string): { key: string; code: string; windowsVirtualKeyCode: number; modifier: number } {
  ...
  if (k === "control" || k === "ctrl") return { key: "Control", code: "ControlLeft", windowsVirtualKeyCode: 17, modifier: 2 };
