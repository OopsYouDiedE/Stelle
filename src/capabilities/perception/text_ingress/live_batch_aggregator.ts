...
export class LiveBatchAggregator {
  ...
  // --- Buffer Management ---
  push(event: TextIngressEvent): void {
    const t = this.now();
  ...
  private acceptWithOverflow(incoming: TextIngressEvent): boolean {
  ...
  private isUrgent(event: TextIngressEvent): boolean {
    return event.kind === "super_chat" || event.kind === "guard" || event.kind === "gift" || event.priority === "high";
  }
}

// === Helpers ===
function priorityScore(event: TextIngressEvent): number {
