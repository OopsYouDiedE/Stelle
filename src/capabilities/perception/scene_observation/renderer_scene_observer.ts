...
export interface SceneRendererStateSource {
  getStatus(): {
    connected: boolean;
    socketCount: number;
    state: {
...
export class SceneObserver {
  private renderer?: SceneRendererStateSource;

  constructor(
    private readonly config: SceneObservationConfig,
    renderer?: SceneRendererStateSource,
  ) {
    this.renderer = renderer;
  }

  setRenderer(renderer?: SceneRendererStateSource): void {
    this.renderer = renderer;
  }
