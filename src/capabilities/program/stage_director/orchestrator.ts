...
export class TopicOrchestrator {
  ...
  ingestEvent(event: ProgramInputEvent): {
    updated: boolean;
    state: TopicState;
    reason: string;
    transition?: { phaseChanged: boolean; from?: TopicPhase; to?: TopicPhase };
  } {
    if (event.kind !== "text" && event.kind !== "super_chat") {
    ...
    if (isUnsafePublicText(text)) {
      return { updated: false, state: this.snapshot(), reason: "moderation_rejected" };
    }
...
export interface ProgramInputEvent {
  id: string;
  source: string;
  kind: "text" | "super_chat" | "gift" | "guard" | "system" | "unknown";
  text: string;
  receivedAt: number;
  priority: "low" | "medium" | "high";
}

function normalizeProgramInput(payload: Record<string, unknown>): ProgramInputEvent {
  const inner = (payload.payload && typeof payload.payload === "object" ? payload.payload : payload) as Record<
    string,
    unknown
  >;
  const kind = String(inner.kind ?? payload.kind ?? "text");
  const metadata =
    payload.metadata && typeof payload.metadata === "object" ? (payload.metadata as Record<string, unknown>) : {};
  return {
    id: String(payload.id ?? `program-event-${Date.now()}`),
    source: String(payload.sourceWindow ?? payload.source ?? "runtime"),
    kind: kind === "super_chat" || kind === "gift" || kind === "guard" || kind === "system" ? kind : "text",
    text: String(inner.text ?? payload.text ?? ""),
    receivedAt: Number(payload.timestamp ?? payload.receivedAt ?? Date.now()),
    priority: metadata.priority === "high" || payload.priority === "high" ? "high" : "medium",
  };
}
...
function isUnsafePublicText(text: string): boolean {
  return /忽略(之前|以上|所有).*(规则|指令)|system prompt|developer message/i.test(text);
}
