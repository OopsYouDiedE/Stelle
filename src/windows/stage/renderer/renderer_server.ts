...
function debugHtml(): string {
  return `<!doctype html>
<html>
<head><meta charset="utf-8"><title>Stelle Debug</title></head>
<body>
<pre id="snapshot">loading</pre>
<script>
fetch('/_debug/api/snapshot').then(r => r.json()).then(v => {
  document.getElementById('snapshot').textContent = JSON.stringify(v.snapshot ?? v, null, 2);
}).catch(e => {
  document.getElementById('snapshot').textContent = String(e);
});
</script>
</body>
</html>`;
