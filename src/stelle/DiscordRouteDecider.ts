...
export function needsVerification(text: string): boolean {
  return /新闻|最新|今天|昨天|刚刚|现在|查证|核实|搜索|来源|真的假的|是否属实|怎么回事|发生了什么|news|latest|source|verify|fact.?check/i.test(
    text
  );
}

export function isLiveAction(text: string): boolean {
  return /直播|推流|obs|live2d|字幕|讲给直播|上播|开播|下播|场景|口型|语音|念出来|读出来|背景/i.test(text);
}

export function isSocialAction(text: string, mentionedOtherUsers: boolean): boolean {
  if (!mentionedOtherUsers) return false;
  return /调戏|逗|吐槽|夸|提醒|cue他|cue她|叫他|叫她|跟他说|跟她说|帮我说|点名/i.test(text);
}

export function isSelfOrSystem(text: string): boolean {
  return /stelle|core mind|大脑|召回|你是谁|你现在在哪|哪个窗口|当前窗口|附着|cursor|inner|人格|性格|主循环|main.?loop/i.test(
    text
  );
}

export function isMemoryOrContinuity(text: string): boolean {
  return /记住|记忆|忘掉|遗忘|以后都|长期|偏好|别再|不要再|下次/i.test(text);
}

function isHighRisk(text: string): boolean {
  return /密码|token|api.?key|密钥|删库|删除全部|封禁|踢出|禁言|人肉|隐私|身份|住址|手机号|自杀|伤害|违法|诈骗|爆破/i.test(
    text
  );
}
