...
export class TopicScriptService {
  ...
  async generateDraft(input: TopicScriptGenerationInput, actor = "system"): Promise<{ draft: TopicScriptDraft; record: TopicScriptRevisionRecord }> {
  ...
  async generateDraftObject(input: TopicScriptGenerationInput): Promise<TopicScriptDraft> {
    ...
    try {
      ...
      console.warn(`[TopicScriptService] LLM draft generation failed; using fallback draft: ${error instanceof Error ? error.message : String(error)}`);
      return fallback;
    }
  }
...
function buildFallbackDraft(input: TopicScriptGenerationInput, now: number): TopicScriptDraft {
  ...
  const sections = phases.map((phase, index): TopicScriptSection => ({
    ...
    host_script: index === 0 ? `今天我们聊「${title}」。我会先抛问题，再根据弹幕分几类观点。` : `我们进入${phase}阶段，先把刚才的弹幕整理成可以继续讨论的问题。`,
...
function normalizeDraft(raw: unknown, fallback: TopicScriptDraft): TopicScriptDraft {
  ...
  return TopicScriptDraftSchema.parse({
    ...
    sections: Array.isArray(value.sections) ? value.sections.map((section, index) => normalizeSection(section, fallback.sections[index] ?? fallback.sections[0]!)) : fallback.sections,
  });
}
