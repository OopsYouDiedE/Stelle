...
describe.skipIf(!hasEvalLlmKeys())("Memory Use Capability Eval", () => {
  it("scores fact use and memory write restraint", async () => {
    ...
    for (const evalCase of cases) {
      ...
      const mustUse = Array.isArray(evalCase.expected.mustUseMemoryKeys)
        ? evalCase.expected.mustUseMemoryKeys.map(String)
        : [];
      const score = summarizeChecks([
        ...
        {
          ok:
            evalCase.expected.shouldWriteMemory === undefined ||
            result.shouldWriteMemory === evalCase.expected.shouldWriteMemory,
