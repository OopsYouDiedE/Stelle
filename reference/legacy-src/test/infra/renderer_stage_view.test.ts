...
describe("formal live renderer view", () => {
  it("keeps /live standalone mode free of control-room panels", async () => {
    const css = await fs.readFile(path.resolve("reference/legacy-src/assets/renderer/client/src/style.css"), "utf8");
  ...
  it("does not ship a hard-coded live topic as the renderer empty state", async () => {
    const html = await fs.readFile(path.resolve("reference/legacy-src/assets/renderer/client/index.html"), "utf8");
