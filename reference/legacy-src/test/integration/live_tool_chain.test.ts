...
describe("Live tool chain integration", () => {
  it("returns StageOutputArbiter decisions with group and sequence metadata", async () => {
    ...
    const decisions = await responder.enqueue("response", "第一句。第二句。", "neutral", {
      groupId: "group-1",
      sequenceStart: 0,
    });

    expect(decisions.map((d) => d.status)).toEqual(["queued", "dropped"]);
    expect(stageOutput.propose).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ groupId: "group-1", sequence: 0 }),
    );
    expect(stageOutput.propose).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ groupId: "group-1", sequence: 1 }),
    );
  });
  ...
  it("should compose final speech from executed tool results for topic requests", async () => {
    ...
    const toolExecute = vi.fn().mockImplementation(async (name: string) => {
      if (name === "memory.search")
        return { ok: true, summary: "tool hit", data: { results: [{ excerpt: "remembered detail" }] } };
    ...
    const generateJson = vi
      .fn()
      .mockImplementationOnce(async (_prompt, _schema, normalize) =>
        normalize({
    ...
    const router = new LiveRouter(
      {
        ...
        llm: {
          generateJson: vi.fn().mockImplementation(async (_prompt, _schema, normalize) =>
            normalize({
              action: "drop_noise",
              emotion: "neutral",
              intensity: 1,
