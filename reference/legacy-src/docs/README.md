...
这份 README 只回答两个问题：如何配置，如何启动。代码结构、项目规范、测试规范和记忆生成规则放在本目录。
...
构建会先打包 `reference/legacy-src/assets/renderer/client/` 的 Live2D renderer，再编译 TypeScript runtime 到 `dist/`。
...
- [项目结构](STRUCTURE.md)：说明每个目录和核心模块的职责。
- [项目规范](PROJECT_CONVENTIONS.md)：编码、格式、模块边界、工具和安全约束。
- [测试规范](TESTING.md)：确定性测试、eval、验证范围和常见失败。
- [记忆生成规范](MEMORY_GENERATION.md)：recent、checkpoint、long-term、proposal 与检索抗偏规则。
- [架构约束](ARCHITECTURE.md)：事件边界、Cursor、Actuator、ToolRegistry 等架构合同。
- [代码地图](CODEBASE_GUIDE.md)：修改代码时的入口、业务流和常见扩展点。
- [运维手册](OPERATIONS.md)：更细的启动、控制页和 debug 路由说明。
- [Topic Script 格式](TOPIC_SCRIPT_FORMAT.md)：直播节目脚本格式。
- [Topic Script Runbook](TOPIC_SCRIPT_RUNBOOK.md)：脚本生成、审核、加载和直播操作流程。
