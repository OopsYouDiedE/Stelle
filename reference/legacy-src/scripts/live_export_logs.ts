...
const sessionsRoot = path.resolve("reference/legacy-src/artifacts/live-sessions");
const outputDir = path.resolve("reference/legacy-src/artifacts/live-support-bundles");
const sessionId = process.argv.find((arg) => arg.startsWith("--session="))?.split("=")[1] ?? (await latestSessionId());

if (!sessionId) {
  console.error("No live session found under reference/legacy-src/artifacts/live-sessions.");
  process.exit(1);
}
