...
import { getAudioCursor } from "../audio/index.js";
...
function inferProvider(baseUrl: string, apiKey: string): string {
...
function resolveDefaultLlmConfig(): ResolvedLlmConfig {
  if (process.env.AISTUDIO_API_KEY || process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY) {
    const apiKey =
      process.env.AISTUDIO_API_KEY ||
      process.env.GEMINI_API_KEY ||
      process.env.GOOGLE_API_KEY ||
    ...
    return {
      ...
      model: process.env.AISTUDIO_MODEL || process.env.GEMINI_MODEL || DEFAULT_MODEL,
      provider: "gemini",
    };
  }
...
function resolveLlmConfig(
  ...
  const fallback = resolveDefaultLlmConfig();
  const cfg = getLlmConfig(guildId, userId);
  const apiKey = String(cfg.api_key || fallback.apiKey);
  const baseUrl = String(cfg.base_url || fallback.baseUrl);
  const model = String(cfg.model || fallback.model);
  return {
    apiKey,
    baseUrl,
    model,
    provider: inferProvider(baseUrl, apiKey),
  };
}

function getLocalClient(guildId: string | null, userId?: string | null): OpenAI {
  const cfg = resolveLlmConfig(guildId, userId);
  ...
  if (!c) {
    console.log(
      `[LLM] provider=${cfg.provider} model=${cfg.model} base_url=${baseUrl} key=${redactKey(apiKey)}`
    );
...
async function synthesizeReplyAudio(input: {
  text: string;
  requester?: User | null;
  channel: TextBasedChannel;
}): Promise<void> {
  const enabled = process.env.DISCORD_TTS_ENABLED?.toLowerCase() !== "false";
  if (!enabled || !input.channel.isSendable()) return;

  const text = input.text.replace(/\s+/g, " ").trim().slice(0, 800);
  if (!text) return;

  const result = await getAudioCursor().submitOutput({
    id: `discord-reply-${Date.now()}`,
    text,
    voice: process.env.KOKORO_TTS_VOICE,
    speed: process.env.KOKORO_TTS_SPEED
      ? Number(process.env.KOKORO_TTS_SPEED)
      : undefined,
    createdAt: Date.now(),
  ...
  if (!result.ok || !result.audioPath) {
    await sendLogEmbed(
      ...
      result.summary,
      Colors.Orange,
      [["Source", `channel=${input.channel.id}\nuser=${input.requester?.id ?? "unknown"}`, false]]
    );
    return;
  }

  await input.channel.send({
    content: input.requester ? `<@${input.requester.id}> 语音版也给你放这儿。` : "语音版也放这儿。",
    files: [new AttachmentBuilder(result.audioPath)],
    allowedMentions: {
      users: input.requester ? [input.requester.id] : [],
      parse: [],
      repliedUser: false,
    },
  });
...
function buildJudgePrompt(isDm: boolean): string {
  ...
  const base = `You are Stelle (User ID: ${botId}). Your current identity and self-reference are always Stelle. You MUST output valid JSON ONLY:
{
...
1. Identity: If Stelle or your ID is mentioned, decisively set fire_now: true. 2. Recognize users by ID, names are just aliases.`;
...
function buildCharacterPrompt(isDm: boolean): string {
  ...
  return `You are ${env} Stelle (User ID: ${getBotId()}). Your name is Stelle.
...
const slashCommands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o
        .setName("model")
        .setDescription("The model name / 模型名称 (e.g. gemini-2.5-flash)")
        .setRequired(true)
    )
    .addStringOption((o) =>
      o
...
client.on(Events.TypingStart, async (t) => {
  ...
  const reports = await stelle.tickCursor(discordCursor.id);
  if (reports.some((report) => report.type === "message_processed")) {
    const cycle = await stelle.runAttentionCycle();
    await notifyMinecraftResults({ client, cycle });
  }
});
