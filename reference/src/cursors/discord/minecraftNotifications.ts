...
export async function notifyMinecraftResults(input: {
  ...
  for (const report of input.cycle.reports) {
    ...
    const content = `${mention}${formatMinecraftResultMessage(report.type, report.summary)}`;
    await channel.send({
      content,
      allowedMentions: {
...
function formatMinecraftResultMessage(type: string, summary: string): string {
  if (type === "error") {
