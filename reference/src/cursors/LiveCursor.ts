...
export class LiveCursor extends BaseCursor {
  ...
  async tick(): Promise<CursorReport[]> {
    ...
    reports.push(
      this.report(
        "live_health",
        status.obs.lastError ? "warning" : "debug",
        `OBS streaming=${status.obs.streaming}`,
        Boolean(status.obs.lastError),
        {
          obs: status.obs,
          stage: status.stage,
  ...
  private async speakQueuedSpeech(text: string, filePrefix: string): Promise<void> {
    if (process.env.LIVE_TTS_ENABLED !== "true") return;
...
function normalizeSpeechText(text: string): string {
  return text.replace(/\s+/g, "").replace(/[锛屻€傦紒锛??锛?銆?.]/g, "");
}
