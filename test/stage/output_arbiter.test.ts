...
import type { OutputIntent, StageOutputRenderer } from "../../src/capabilities/expression/stage_output/types.js";
...
describe("StageOutputArbiter", () => {
  ...
  it("drops expired queued output without rendering it", async () => {
    ...
    expect(mockRenderer.render).toHaveBeenCalledTimes(1);
    expect(mockRenderer.render).not.toHaveBeenCalledWith(
  ...
  it("publishes stage.output.dropped when queued output expires", async () => {
    ...
    const dropped = eventBus
      .getHistory()
      .find((event) => event.type === "stage.output.dropped" && event.payload.intent.id === "expired-visible");
    expect(dropped?.payload.reason).toBe("expired");
    expect(mockRenderer.render).not.toHaveBeenCalledWith(
      expect.objectContaining({ id: "expired-visible" }),
      expect.any(AbortSignal),
    );
  });
  ...
  it("mutes TTS on accepted intents", async () => {
    ...
    expect(mockRenderer.render).toHaveBeenCalledWith(
      expect.objectContaining({ output: expect.objectContaining({ tts: false }) }),
      expect.any(AbortSignal),
    );
  });
});
