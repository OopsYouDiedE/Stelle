...
describe("DefaultResearchAgenda", () => {
  ...
  it("three related medium signals merge into one topic", async () => {
    ...
    const now = Date.now();
    const signals: CognitiveSignal[] = [
      { id: "s1", source: "live_danmaku", kind: "stream", summary: "Lag report 1", timestamp: now, impactScore: 1, salience: "medium" },
      { id: "s2", source: "live_danmaku", kind: "stream", summary: "Lag report 2", timestamp: now + 1, impactScore: 1, salience: "medium" },
      { id: "s3", source: "live_danmaku", kind: "stream", summary: "Lag report 3", timestamp: now + 2, impactScore: 1, salience: "medium" },
    ];
  ...
  it("expired topics close and are excluded from activeTopics", async () => {
    ...
    await agenda.update([{
      id: "s1", source: "system", kind: "theme", summary: "Temporary topic", timestamp: now, impactScore: 1, salience: "high"
    }], mockSelf, now);
  ...
  it("hydrate restores topics from memory", () => {
    const agenda = new DefaultResearchAgenda();
    const mockTopics = [
      { id: "t1", title: "Saved Topic", status: "active", evidence: [] }
    ] as any;
